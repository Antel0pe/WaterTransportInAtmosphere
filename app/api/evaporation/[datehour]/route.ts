 ...
 */
function parseMoisturePngName(name: string): string | null {
...
export async function GET(
  ...
  const imgDir = path.join(dataRootPath, "evap_rgb_instant_clim_anom");
  ...
  const keys = files
    .map(parseMoisturePngName)
    .filter((x): x is string => x !== null)
    .sort(); // lexicographic works with this filename pattern

  if (keys.length === 0) {
    return NextResponse.json(
      { error: "no moisture transport images available" },
      { status: 500 }
    );
  }
  ...
  return new NextResponse(new Uint8Array(buf), {
    ...
    headers: {
      ...
      // optional: cache-control (adjust if you want caching)
