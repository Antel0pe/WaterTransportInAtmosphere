...
function parsePvPngName(name: string): string | null {
...
export async function GET(
  ...
  const imgDir = path.join(process.cwd(), "public", "potential-vorticity-rg", String(pressure));
  ...
  const keys = files.map(parsePvPngName).filter((x): x is string => x !== null).sort();

  if (keys.length === 0) {
    return NextResponse.json({ error: "no potential vorticity images available" }, { status: 500 });
  }
